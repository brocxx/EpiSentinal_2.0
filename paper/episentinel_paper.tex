...
\author{%
  \IEEEauthorblockN{Ekansh Nandan Sharma,\; GN Disha,\; Ishan Mahajan}
  \IEEEauthorblockA{%
    \textit{Department of Computer Science Engineering}\\
    RV College of Engineering, Bengaluru, India\\
    \texttt{\{ekanshnandans.cs25, gndisha.cs25,
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{scope}
      % Feature list (bottom-to-top in bar chart order)
      \def\barlabel{{"cases\_per\_100k",
                      "rainfall\_lag1",
                      "cases\_lag3",
                      "week\_sin/cos",
                      "cases\_roll2\_mean",
                      "temp\_mean\_lag2",
                      "cases\_roll4\_mean"}}
      \def\barval{{0.071, 0.072, 0.102, 0.118, 0.120, 0.124, 0.278}}
      \def\barcol{{"blue!45","blue!45","teal!60","orange!55","teal!60","blue!45","teal!80"}}

      \def\maxval{0.278}
      \def\barwidth{3.8cm}
      \def\rowheight{0.50cm}
      \def\labelwidth{2.7cm}

      \foreach \i in {0,...,6} {
        \pgfmathsetmacro{\y}{\i*\rowheight}
        \pgfmathsetmacro{\bw}{\barval[\i]/\maxval*\barwidth}
        \pgfmathparse{\barlabel[\i]}\let\lbl\pgfmathresult
        \pgfmathparse{\barcol[\i]}\let\col\pgfmathresult

        % Label
        \node[anchor=east, font=\scriptsize\ttfamily,
              text width=\labelwidth, align=right]
          at (0, \y) {\lbl};

        % Bar
        \fill[\col, rounded corners=1pt]
          (0.05cm, \y-0.17cm) rectangle (\bw+0.05cm, \y+0.17cm);

        % Value text
        \node[anchor=west, font=\scriptsize, color=black!70]
          at (\bw+0.08cm, \y) {\pgfmathprintnumber[fixed,precision=3]{\barval[\i]}};
      }

      % X-axis label
      \node[font=\scriptsize, color=black!60] at (1.8cm, -0.4cm)
        {Mean $|\phi_i|$ (SHAP Value)};
    \end{scope}
  \end{tikzpicture}
